...
class SURP(ccobra.CCobraModel):
    ...
    def predictS(self, item, **kwargs):
        ...
        if kwargs['truthful']:
            threshold = self.Cparameter['Cr'] + self.Cparameter['Mr'] * kwargs['crt'] +  abs(kwargs['panasPos'] + kwargs['panasNeg'])*self.parameter['mood_R']
        if not kwargs['truthful']:
            threshold = self.Cparameter['Cf'] + self.Cparameter['Mf'] * kwargs['crt'] +  abs(kwargs['panasPos'] + kwargs['panasNeg'])*self.parameter['mood_F']
        return threshold
