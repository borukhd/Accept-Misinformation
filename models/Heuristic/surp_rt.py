...
class SURP(ccobra.CCobraModel):
    ...
    """
    def __init__(self, name='WMSupprByMood&RT'):
    ...
    def predictS(self, item, **kwargs):
        ...
        return threshold + abs(kwargs['panasPos'] + kwargs['panasNeg'])*self.parameter['mood'] + kwargs['reaction_time']*self.parameter['!alpha']
        ...
        if self.name in OptPars.parsPerPers.keys() and dataset[0]['id'] in OptPars.parsPerPers[self.name]:
            ...
            if len(self.parameter.keys()) > 1:
                with np.errstate(divide='ignore'):
                    bounds = [(-5,5)*(len(self.parameter.keys())-1)]
