...
class RT(ccobra.CCobraModel):
    ...
    def __init__(self, name='CR&time', commands = []):
        ...
        optdict = {'alpha_R': -0.015201555811670677, 'alpha_F': -0.015201555811670677}
    ...
    def predictS(self, item, **kwargs):
        ...
        if kwargs['truthful']:
            threshold = self.Cparameter['Cr'] + self.Cparameter['Mr'] * kwargs['crt'] + kwargs['reaction_time']*self.parameter['alpha_R']
        if not kwargs['truthful']:
            threshold = self.Cparameter['Cf'] + self.Cparameter['Mf'] * kwargs['crt'] + kwargs['reaction_time']*self.parameter['alpha_F']
