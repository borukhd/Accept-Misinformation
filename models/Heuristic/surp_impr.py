...
from models.LinearCombination.optimizationParameters import RandomDisplacementBounds
import os
import sys
PACKAGE_PARENT = '..'
...
class SURP_Impr(ccobra.CCobraModel):
    globalpars = {}
    """ News reasoning CCOBRA implementation.
    """
    def __init__(self, name='WMImprByMood'):
    ...
    def predictS(self, item, **kwargs):
        ...
        if kwargs['truthful']:
            threshold = self.Cparameter['Cr'] + self.Cparameter['Mr'] * kwargs['crt'] + (kwargs['panasPos'] - kwargs['panasNeg'])*self.parameter['mood_R']
        if not kwargs['truthful']:
            threshold = self.Cparameter['Cf'] + self.Cparameter['Mf'] * kwargs['crt'] + (kwargs['panasPos'] - kwargs['panasNeg'])*self.parameter['mood_F']
        return threshold 
